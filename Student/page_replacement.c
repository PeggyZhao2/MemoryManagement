...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        fte_t* ftable_entry = (fte_t*) (frame_table + victim_pfn);
        pte_t* page_table = (pte_t*) (mem + (ftable_entry -> process -> saved_ptbr) * PAGE_SIZE);
        pte_t* ptable_entry = &page_table[ftable_entry -> vpn];

        if (ptable_entry -> dirty) {
            swap_write(ptable_entry, &mem[victim_pfn * PAGE_SIZE]);
            ptable_entry -> dirty = 0;
            stats.writebacks++;
        }
        ptable_entry -> valid = 0;
        ftable_entry -> mapped = 0;
    }
    /* Return the pfn */
    return victim_pfn;
}
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        while (1) {
           if (!frame_table[last_pfn % num_entries].protected) {
               if (!frame_table[last_pfn % num_entries].referenced) {
                   last_pfn++;
                   return last_pfn-1;
               }
           }
           frame_table[last_pfn % num_entries].referenced = 0;
           last_pfn = (last_pfn + 1) % num_entries;
        }              
    }
