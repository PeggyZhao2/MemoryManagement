...
void proc_init(pcb_t *proc) {
    ...
    memset(mem + (victim_pfn * PAGE_SIZE), 0, PAGE_SIZE);
    proc -> saved_ptbr = victim_pfn;
    fte_t* ft_ent = (fte_t*) (frame_table + victim_pfn);
    ft_ent -> protected = 1;
    ft_ent -> process = proc;
...
void proc_cleanup(pcb_t *proc) {
    ...
    pte_t* page_table = (pte_t*) (mem + (pfn * PAGE_SIZE));
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        ...
        if (page_table_entry -> valid) {
            ...
            frame_table[page_table_entry -> pfn].protected = 0;
        }
